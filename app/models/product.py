...
class Product:
    def __init__(self, product_id, reviews=[], product_name="",  stats = {}):
        ...
        self.stats = stats
    def __str__(self):
        return f"""productid : {self.product_id}
    ...
    def extract_reviews(self):
        ...
        #print(response.text)
            if response.status_code == 200:
                page_dom = BeautifulSoup(response.text, "html.parser")
                reviews = page_dom.select(".js_product-review:not(.user-post--highlight)")
                print(len(reviews))
                for review in reviews:
                        single_review = Review()
                        self.reviews.append(single_review).extract_featers(review).transform()
    ...
    def calculate_stats(self):
        ...
        self.stats["avarage_stars"] = round(reviews.stars.mean(),2)
        return self
